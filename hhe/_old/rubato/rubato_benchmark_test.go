...
func BenchmarkRubato(b *testing.B) {
	...
	for _, tc := range rubato2.TestsVector {
		benchHERubato(tc, b)
	}
...
func benchHERubato(tc rubato2.TestContext, b *testing.B) {
	...
	b.Run("Rubato/SymKeyStream", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			for i := 0; i < heRubato.N; i++ {
				symRub := rubato2.NewRubato(tc.Key, tc.Params)
