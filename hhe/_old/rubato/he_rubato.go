...
func (hR *HERubato) InitParams(paramIndex int, symParams rubato.Parameter, plainSize int) {
	...
	hR.hbtpParams = ckks_fv2.RtFRubatoParams[0] // using Rubato 128af
	...
	hR.rubatoModDown = ckks_fv2.RubatoModDownParams[paramIndex].CipherModDown
	hR.stcModDown = ckks_fv2.RubatoModDownParams[paramIndex].StCModDown
}

func (hR *HERubato) HEKeyGen() {
	hR.keyGenerator = ckks_fv2.NewKeyGenerator(hR.params)
	hR.sk, hR.pk = hR.keyGenerator.GenKeyPairSparse(hR.hbtpParams.H)

	hR.fvEncoder = ckks_fv2.NewMFVEncoder(hR.params)
	hR.ckksEncoder = ckks_fv2.NewCKKSEncoder(hR.params)
	hR.fvEncryptor = ckks_fv2.NewMFVEncryptorFromPk(hR.params, hR.pk)
	hR.ckksDecryptor = ckks_fv2.NewCKKSDecryptor(hR.params, hR.sk)
}
...
func (hR *HERubato) HalfBootKeyGen() {
	...
	hR.hbtpKey = ckks_fv2.BootstrappingKey{Rlk: hR.rlk, Rtks: hR.rotkeys}
}

func (hR *HERubato) InitHalfBootstrapper() {
	var err error
	if hR.hbtp, err = ckks_fv2.NewHalfBootstrapper(hR.params, hR.hbtpParams, hR.hbtpKey); err != nil {
		panic(err)
	}
}

func (hR *HERubato) InitEvaluator() {
	hR.fvEvaluator = ckks_fv2.NewMFVEvaluator(hR.params, ckks_fv2.EvaluationKey{Rlk: hR.rlk, Rtks: hR.rotkeys}, hR.pDcds)
}
...
func (hR *HERubato) EncodeEncrypt(keystream [][]uint64) {
	hR.plainCKKSRingTs = make([]*ckks_fv2.PlaintextRingT, hR.outSize)
	for s := 0; s < hR.outSize; s++ {
...
func (hR *HERubato) ScaleUp() {
	hR.plaintexts = make([]*ckks_fv2.Plaintext, hR.outSize)
	for s := 0; s < hR.outSize; s++ {
		hR.plaintexts[s] = ckks_fv2.NewPlaintextFVLvl(hR.params, 0)
		hR.fvEncoder.FVScaleUp(hR.plainCKKSRingTs[s], hR.plaintexts[s])
	}
}

func (hR *HERubato) InitFvRubato() MFVRubato {
	hR.fvRub = NewMFVRubato(hR.paramIndex, hR.params, hR.fvEncoder, hR.fvEncryptor,
...
func (hR *HERubato) GetFvKeyStreams(nonces [][]byte, counter []byte) []*ckks_fv2.Ciphertext {
...
func (hR *HERubato) ScaleCiphertext(fvKeyStreams []*ckks_fv2.Ciphertext) {
	hR.ciphertext = ckks_fv2.NewCiphertextFVLvl(hR.params, 1, 0)
...
// and returns a ciphertext at level MaxLevel - k, where k is the depth of the aes_bootstrapping circuit.
// Difference from the aes_bootstrapping is that the last StC is missing.
...
func (hR *HERubato) HalfBoot() *ckks_fv2.Ciphertext {
