...
func NewRtBCipher(key []uint8, param_ ckks.Parameters, btpParams_ aesbootstrapper.Parameters, btpKey_ *aesbootstrapper.EvaluationKeys, encoder_ *ckks.Encoder, encryptor_ *rlwe.Encryptor, decryptor_ *rlwe.Decryptor) (rtb *RtBCipher, err error) {
	eval, err := aesbootstrapper.NewEvaluator(btpParams_, btpKey_)
	if err != nil {
...
func BootstrapCipher(eval *aesbootstrapper.Evaluator, ct *rlwe.Ciphertext) (ctOut *rlwe.Ciphertext) {
	// for ct.Level() > 1 {
	// 	rtb.DropLevel(ct, 1)
	// }
...
// func BootReEnc( eval *aes_bootstrapping.Evaluator, decryptor rlwe.Decryptor, ct *rlwe.Ciphertext ) *rlwe.Ciphertext {
...
func PowerOf2(eval *aesbootstrapper.Evaluator, op *rlwe.Ciphertext, logPow2 int, opOut *rlwe.Ciphertext) {
...
func PowerNew(eval *aesbootstrapper.Evaluator, op *rlwe.Ciphertext, degree int) (opOut *rlwe.Ciphertext) {
...
// key is necessary when degree > 2.
func Power(eval *aesbootstrapper.Evaluator, op *rlwe.Ciphertext, degree int, opOut *rlwe.Ciphertext) {
...
func XOR(eval *aesbootstrapper.Evaluator, ct0, ct1 *rlwe.Ciphertext, ctOut *rlwe.Ciphertext) {
	// (x-y)^2
...
// XORNew //boolean function constructed from Heaan using CKKS scheme
func XORNew(eval *aesbootstrapper.Evaluator, ct0, ct1 *rlwe.Ciphertext) (ctOut *rlwe.Ciphertext) {
	// (x-y)^2
...
func NOT(eval *aesbootstrapper.Evaluator, ct0 *rlwe.Ciphertext, ctOut *rlwe.Ciphertext) {
	// (1-x)
...
func NOTNew(eval *aesbootstrapper.Evaluator, ct0 *rlwe.Ciphertext) (ctOut *rlwe.Ciphertext) {
	// (1-x)
	if ct0.Degree() != ctOut.Degree() {
		panic("cannot Negate: invalid receiver Ciphertext does not match input Ciphertext degree")
	}
...
func AND(eval *aesbootstrapper.Evaluator, ct0, ct1 *rlwe.Ciphertext, ctOut *rlwe.Ciphertext) {
	// xy
	eval.MulRelin(ct0, ct1, ctOut)
	eval.Rescale(ctOut, ctOut)
}

func ANDNew(eval *aesbootstrapper.Evaluator, ct0, ct1 *rlwe.Ciphertext) (ctOut *rlwe.Ciphertext) {
	// xy
...
func OR(eval *aesbootstrapper.Evaluator, ct0, ct1 *rlwe.Ciphertext, Out *rlwe.Ciphertext) {
	//x + y - x\cdot y
...
func ORNew(eval *aesbootstrapper.Evaluator, ct0, ct1 *rlwe.Ciphertext) (ctOut *rlwe.Ciphertext) {
	//x + y - x\cdot y
	ctOut, err := eval.MulRelinNew(ct0, ct1)
	if err != nil {
		panic(err)
...
func CleanReal(eval *aesbootstrapper.Evaluator, ct *rlwe.Ciphertext) {
...
func BinaryTreeAdd(eval *aesbootstrapper.Evaluator, cts []*rlwe.Ciphertext) (ctOut *rlwe.Ciphertext) {
...
func ConstructNumberFromBits(eval *aesbootstrapper.Evaluator, cts []*rlwe.Ciphertext, error int) (ctOut *rlwe.Ciphertext) {
