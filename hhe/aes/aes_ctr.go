...
func NewAESCtr(key_ []uint8, params_ ckks.Parameters, btpParams_ aesbootstrapper.Parameters, btpKey_ *aesbootstrapper.EvaluationKeys, encoder_ *ckks.Encoder, encryptor_ *rlwe.Encryptor, decryptor_ *rlwe.Decryptor, iv_ []byte) (*AESCtr, error) {
...
func (aes *AESCtr) RoundFunction(state []*rlwe.Ciphertext, roundKey []*rlwe.Ciphertext) {
	...
	// Parallel processing for aes_bootstrapping and cleaning tensor
	for i := 0; i < 128; i++ {
		go func(i int) {
			evalCopy := aes.Evaluator.ShallowCopy()
...
func (aes *AESCtr) coefficientMultMonomial(eval *aesbootstrapper.Evaluator, mon []*rlwe.Ciphertext, coeffArr []int, pos int) (ctOut *rlwe.Ciphertext) {
...
func (aes *AESCtr) aesSubbyteLUT(eval *aesbootstrapper.Evaluator, SBoxIn []*rlwe.Ciphertext) {
...
func GF2FieldMul(eval *aesbootstrapper.Evaluator, x []*rlwe.Ciphertext) {
