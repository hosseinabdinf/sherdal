...
type MFVRubato interface {
	Crypt(nonce [][]byte, counter []byte, kCt []*ckks_fv.Ciphertext, rubatoModDown []int) []*ckks_fv.Ciphertext
	CryptNoModSwitch(nonce [][]byte, counter []byte, kCt []*ckks_fv.Ciphertext) []*ckks_fv.Ciphertext
	CryptAutoModSwitch(nonce [][]byte, counter []byte, kCt []*ckks_fv.Ciphertext, noiseEstimator ckks_fv.MFVNoiseEstimator) (res []*ckks_fv.Ciphertext, rubatoModDown []int)
...
func NewMFVRubato(rubatoParam int, params *ckks_fv.Parameters, encoder ckks_fv.MFVEncoder, encryptor ckks_fv.MFVEncryptor, evaluator ckks_fv.MFVEvaluator, nbInitModDown int) MFVRubato {
	...
	rubato.stCt = make([]*ckks_fv.Ciphertext, rubato.blocksize)
	rubato.mkCt = make([]*ckks_fv.Ciphertext, rubato.blocksize)
	rubato.rkCt = make([]*ckks_fv.Ciphertext, rubato.blocksize)
	rubato.rcPt = make([]*ckks_fv.PlaintextMul, rubato.blocksize)
...
func (rubato *mfvRubato) Reset(nbInitModDown int) {
	...
	for i := 0; i < rubato.blocksize; i++ {
		...
		icPT := ckks_fv.NewPlaintextFV(rubato.params)
	...
	for r := 0; r <= rubato.numRound; r++ {
		for i := 0; i < rubato.blocksize; i++ {
			for slot := 0; slot < slots; slot++ {
				rubato.rc[r][i][slot] = ckks_fv.SampleZqx(rubato.xof[slot], rubato.params.PlainModulus())
			}
		}
	}
...
func (rubato *mfvRubato) findBudgetInfo(noiseEstimator ckks_fv.MFVNoiseEstimator) (maxInvBudget, minErrorBits int) {
...
func (rubato *mfvRubato) modSwitchAuto(round int, noiseEstimator ckks_fv.MFVNoiseEstimator, rubatoModDown []int) {
...
func (rubato *mfvRubato) Crypt(nonce [][]byte, counter []byte, kCt []*ckks_fv.Ciphertext, rubatoModDown []int) []*ckks_fv.Ciphertext {
...
func (rubato *mfvRubato) EncKey(key []uint64) (res []*ckks_fv.Ciphertext) {
	slots := rubato.slots
	res = make([]*ckks_fv.Ciphertext, rubato.blocksize)
	...
	for i := 0; i < rubato.blocksize; i++ {
		...
		keyPt := ckks_fv.NewPlaintextFV(rubato.params)
