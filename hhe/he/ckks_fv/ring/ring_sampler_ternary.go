...
func NewTernarySamplerSparse(prng sampling.PRNG, baseRing *Ring, hw int, montgomery bool) *TernarySampler {
	ternarySampler := new(TernarySampler)
	ternarySampler.baseRing = baseRing
	ternarySampler.prng = prng
...
func (ts *TernarySampler) sampleProba(lvl int, pol *Poly) {
	...
	if ts.p == 0.5 {
		...
		_, err := ts.prng.Read(randomBytesCoeffs)
		utils.HandleError(err)

		_, err = ts.prng.Read(randomBytesSign)
		utils.HandleError(err)
		...
		_, err := ts.prng.Read(randomBytes)
		utils.HandleError(err)
