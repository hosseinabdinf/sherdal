...
func genTestParams(defaultParams *Parameters) (testContext *testParams, err error) {
	...
	if testContext.prng, err = sampling.NewPRNG(); err != nil {
		return nil, err
	}
...
func testPRNG(testContext *testParams, t *testing.T) {
	...
	t.Run(testString("PRNG/", testContext.ringQ), func(t *testing.T) {
		prng1, err := sampling.NewKeyedPRNG(nil)
		if err != nil {
			panic(err)
		}
		prng2, err := sampling.NewKeyedPRNG(nil)
		if err != nil {
			panic(err)
		}

		n, err := prng1.Read(sum)
		utils.HandleError(err)
		fmt.Println("n=", n)
		n, err = prng2.Read(sum)
		utils.HandleError(err)
		fmt.Println("n=", n)
