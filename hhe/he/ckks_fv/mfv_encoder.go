...
func NewMFVEncoder(params *Parameters) MFVEncoder {
	...
	for i := 0; i < rowSize; i++ {
		...
		indexMatrix[i] = utils.BitReverse64(uint64(index1), logN)
		indexMatrix[i|rowSize] = utils.BitReverse64(uint64(index2), logN)
	...
	for i := 0; i < rowSize; i++ {
		...
		indexMatrixSmall[i] = utils.BitReverse64(uint64(index1), logFVSlots)
		indexMatrixSmall[i|rowSize] = utils.BitReverse64(uint64(index2), logFVSlots)
