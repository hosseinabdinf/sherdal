...
type MFVHera interface {
	Crypt(nonce [][]byte, kCt []*ckks_fv.Ciphertext, heraModDown []int) []*ckks_fv.Ciphertext
	CryptNoModSwitch(nonce [][]byte, kCt []*ckks_fv.Ciphertext) []*ckks_fv.Ciphertext
	CryptAutoModSwitch(nonce [][]byte, kCt []*ckks_fv.Ciphertext, noiseEstimator ckks_fv.MFVNoiseEstimator) (res []*ckks_fv.Ciphertext, heraModDown []int)
	Reset(nbInitModDown int)
	EncKey(key []uint64) (res []*ckks_fv.Ciphertext)
}
...
type mfvHera struct {
	...
	encoder   ckks_fv.MFVEncoder
	encryptor ckks_fv.MFVEncryptor
	evaluator ckks_fv.MFVEvaluator

	stCt []*ckks_fv.Ciphertext
	mkCt []*ckks_fv.Ciphertext
	rkCt []*ckks_fv.Ciphertext   // Buffer for round key
	rc   [][][]uint64            // RoundConstants[round][state][slot]
	rcPt []*ckks_fv.PlaintextMul // Buffer for round constants
	xof  []sha3.ShakeHash
}

func NewMFVHera(numRound int, params *ckks_fv.Parameters, encoder ckks_fv.MFVEncoder, encryptor ckks_fv.MFVEncryptor, evaluator ckks_fv.MFVEvaluator, nbInitModDown int) MFVHera {
	hera := new(mfvHera)
	...
	hera.stCt = make([]*ckks_fv.Ciphertext, 16)
	hera.mkCt = make([]*ckks_fv.Ciphertext, 16)
	hera.rkCt = make([]*ckks_fv.Ciphertext, 16)
	hera.rcPt = make([]*ckks_fv.PlaintextMul, 16)
	...
	for i := 0; i < 16; i++ {
		...
		icPT := ckks_fv.NewPlaintextFV(params)
...
func (hera *mfvHera) Reset(nbInitModDown int) {
	...
	for i := 0; i < 16; i++ {
		...
		icPT := ckks_fv.NewPlaintextFV(hera.params)
	...
	for r := 0; r <= hera.numRound; r++ {
		for st := 0; st < 16; st++ {
			for slot := 0; slot < slots; slot++ {
				hera.rc[r][st][slot] = ckks_fv.SampleZqx(hera.xof[slot], hera.params.PlainModulus())
			}
		}
	}
...
func (hera *mfvHera) findBudgetInfo(noiseEstimator ckks_fv.MFVNoiseEstimator) (maxInvBudget, minErrorBits int) {
...
func (hera *mfvHera) modSwitchAuto(round int, noiseEstimator ckks_fv.MFVNoiseEstimator, heraModDown []int) {
...
func (hera *mfvHera) CryptNoModSwitch(nonce [][]byte, kCt []*ckks_fv.Ciphertext) []*ckks_fv.Ciphertext {
	for st := 0; st < 16; st++ {
...
func (hera *mfvHera) CryptAutoModSwitch(nonce [][]byte, kCt []*ckks_fv.Ciphertext, noiseEstimator ckks_fv.MFVNoiseEstimator) ([]*ckks_fv.Ciphertext, []int) {
...
func (hera *mfvHera) Crypt(nonce [][]byte, kCt []*ckks_fv.Ciphertext, heraModDown []int) []*ckks_fv.Ciphertext {
...
func (hera *mfvHera) addRoundKey(round int, reduce bool) {
	...
	for st := 0; st < 16; st++ {
		hera.rcPt[st] = ckks_fv.NewPlaintextMulLvl(hera.params, hera.stCt[st].Level())
...
func (hera *mfvHera) EncKey(key []uint64) (res []*ckks_fv.Ciphertext) {
	slots := hera.slots
	res = make([]*ckks_fv.Ciphertext, 16)
	...
	for i := 0; i < 16; i++ {
		...
		keyPt := ckks_fv.NewPlaintextFV(hera.params)
