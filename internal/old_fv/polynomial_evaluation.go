...
func (eval *ckksEvaluator) EvaluatePoly(ct0 *Ciphertext, pol *Poly, targetScale float64) (opOut *Ciphertext, err error) {
...
func (eval *ckksEvaluator) EvaluateCheby(op *Ciphertext, cheby *ChebyshevInterpolation, tartetScale float64) (opOut *Ciphertext, err error) {
...
func computePowerBasis(n int, C map[int]*Ciphertext, evaluator *ckksEvaluator) (err error) {
...
func computePowerBasisCheby(n int, C map[int]*Ciphertext, evaluator *ckksEvaluator) (err error) {
...
func recurse(targetScale float64, logSplit, logDegree int, coeffs *Poly, C map[int]*Ciphertext, evaluator *ckksEvaluator) (res *Ciphertext, err error) {
...
func recurseCheby(targetScale float64, logSplit, logDegree int, coeffs *Poly, C map[int]*Ciphertext, evaluator *ckksEvaluator) (res *Ciphertext, err error) {
...
func evaluatePolyFromPowerBasis(targetScale float64, coeffs *Poly, C map[int]*Ciphertext, evaluator *ckksEvaluator) (res *Ciphertext, err error) {

	if coeffs.Degree() == 0 {

		res = NewCiphertextCKKS(evaluator.params, 1, C[1].Level(), targetScale)
	...
	res = NewCiphertextCKKS(evaluator.params, 1, C[coeffs.Degree()].Level(), ctScale)
