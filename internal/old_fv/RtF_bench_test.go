...
func benchmarkRtFHera(b *testing.B, name string, numRound int, paramIndex int, radix int, fullCoeffs bool) {
	...
	// are available in github.com/smilecjf/lattigo/v2/old_fv/rtf_params
	...
	hera = hera2.NewMFVHera(numRound, params, fvEncoder, fvEncryptor, fvEvaluator, heraModDown[0])
	kCt := hera.EncKey(key)
...
func benchmarkRtFRubato(b *testing.B, rubatoParam int) {
	...
	// Rubato parameter
	blocksize := rubato2.RubatoParams[rubatoParam].Blocksize
	outputsize := blocksize - 4
	numRound := rubato2.RubatoParams[rubatoParam].NumRound
	plainModulus := rubato2.RubatoParams[rubatoParam].PlainModulus
	sigma := rubato2.RubatoParams[rubatoParam].Sigma
	...
	// FV Keystream
	rubato = rubato2.NewMFVRubato(rubatoParam, params, fvEncoder, fvEncryptor, fvEvaluator, rubatoModDown[0])
