...
// ParametersLiteral is a struct to parameterize the aes_bootstrapping parameters.
// The ParametersLiteral struct an unchecked struct that is given to the method [NewParametersFromLiteral] to validate them
// and create the aes_bootstrapping [Parameters] struct, which is used to instantiate a Bootstrapper.
// This struct contains only optional fields.
// The default aes_bootstrapping (with no optional field) has
...
// =====================================
//
// LogN: the log2 of the ring degree of the aes_bootstrapping parameters. The default value is 16.
//
// LogP: the log2 of the auxiliary primes during the key-switching operation of the aes_bootstrapping parameters.
// The default value is [61]*max(1, floor(sqrt(#Qi))).
//
// Xs: the distribution of the secret-key used to generate the aes_bootstrapping evaluation keys.
// The default value is ring.Ternary{H: 192}.
//
// Xe: the distribution of the error sampled to generate the aes_bootstrapping evaluation keys.
...
//
//	The user can set this value to 0 to use the regular aes_bootstrapping circuit without the ephemeral secret encapsulation.
//	Be aware that doing so will impact the security, precision, and failure probability of the aes_bootstrapping circuit.
//	See https://eprint.iacr.org/2022/024 for more information.
//
// IterationsParameters : by treating the aes_bootstrapping as a black box with precision logprec, we can construct a aes_bootstrapping of precision ~k*logprec by iteration (see https://eprint.iacr.org/2022/1167).
//   - BootstrappingPrecision: []float64, the list of iterations (after the initial aes_bootstrapping) given by the expected precision of each previous iteration.
//   - ReservedPrimeBitSize: the size of the reserved prime for the scaling after the initial aes_bootstrapping.
//
// For example: &aes_bootstrapping.IterationsParameters{BootstrappingPrecision: []float64{16}, ReservedPrimeBitSize: 16} will define a two iteration aes_bootstrapping (the first iteration being the initial aes_bootstrapping)
// with a additional prime close to 2^{16} reserved for the scaling of the error during the second iteration.
//
// Here is an example for a two iterations aes_bootstrapping of an input message mod [logq0=55, logq1=45] with scaling factor 2^{90}:
//
// INPUT:
...
//
// Notes:
//   - The aes_bootstrapping precision cannot exceed the original input ciphertext precision.
//   - Although the rescalings of 2) and 7) are approximate, we can ignore them and treat them as being part of the aes_bootstrapping error
...
//
//		This ratio directly impacts the precision of the aes_bootstrapping.
...
//	 a non zero value (i.e. 5 or 7). This will greatly improve the precision of the aes_bootstrapping, at the expense of slightly increasing its depth.
//
...
const (
	...
	Custom          = CircuitOrder(2) // Custom order (e.g. partial aes_bootstrapping), disables checks.
)

const (
	// DefaultLogN is the default ring degree for the aes_bootstrapping.
	DefaultLogN = 16
	...
	// DefaultIterations is the default number of aes_bootstrapping iterations.
	DefaultIterations = 1
...
var (
	// DefaultXs is the default secret distribution of the aes_bootstrapping parameters.
	DefaultXs = ring.Ternary{H: 192}
	// DefaultXe is the default error distribution of the aes_bootstrapping parameters.
...
// aes_bootstrapping circuit of the target [ParametersLiteral].
