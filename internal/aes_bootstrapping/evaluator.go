...
// the polynomial approximation, and the keys for the aes_bootstrapping.
// It is used to evaluate the aes_bootstrapping circuit on single ciphertexts.
...
// Depth returns the multiplicative depth (number of levels consumed) of the aes_bootstrapping circuit.
func (eval Evaluator) Depth() int {
	return eval.BootstrappingParameters.MaxLevel() - eval.ResidualParameters.MaxLevel()
}

// OutputLevel returns the output level after the evaluation of the aes_bootstrapping circuit.
...
// Evaluate re-encrypts a ciphertext to a ciphertext at MaxLevel - k where k is the depth of the aes_bootstrapping circuit.
...
// The message ratio is an optional field in the aes_bootstrapping parameters, by default it set to 2^{LogMessageRatio = 8}.
// See the aes_bootstrapping parameters for more information about the message ratio or other parameters related to the aes_bootstrapping.
...
func (eval Evaluator) ModUp(ctIn *rlwe.Ciphertext) (ctOut *rlwe.Ciphertext, err error) {
	...
	if eval.EvkSparseToDense != nil {
		...
		// Scale the message from Q0/|m| to QL/|m|, where QL is the largest modulus used during the aes_bootstrapping.
