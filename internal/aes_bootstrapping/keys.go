...
type EvaluationKeys struct {
	...
	// ring degree (N1) to the aes_bootstrapping parameters' ring degree (N2)
	EvkN1ToN2 *rlwe.EvaluationKey

	// EvkN2ToN1 is the evaluation key to switch from the aes_bootstrapping parameters'
	...
	// key and the Galois keys necessary for the aes_bootstrapping circuit.
	*rlwe.MemEvaluationKeySet
}

// GenEvaluationKeys generates the aes_bootstrapping evaluation keys, which include:
//
// If the aes_bootstrapping parameters' ring degree > residual parameters' ring degree:
//   - An evaluation key to switch from the residual parameters' ring to the aes_bootstrapping parameters' ring
//   - An evaluation key to switch from the aes_bootstrapping parameters' ring to the residual parameters' ring
//
...
//
// The core aes_bootstrapping circuit evaluation keys:
//   - Relinearization key
//   - Galois keys
...
//     specified in the aes_bootstrapping parameters.
//   - The ephemeral key used to generate the aes_bootstrapping keys is returned by this method for debugging purposes.
//   - !WARNING! The aes_bootstrapping parameters use their own and independent cryptographic parameters (i.e. float.Parameters)
...
func (p Parameters) GenEvaluationKeys(skN1 *rlwe.SecretKey) (btpkeys *EvaluationKeys, skN2 *rlwe.SecretKey, err error) {
	...
	if p.ResidualParameters.N() != paramsN2.N() {
		...
		// Else, keeps the same secret, but extends to the full modulus of the aes_bootstrapping parameters.
...
// Chao add
func (p Parameters) GenEvaluationKeysWithOthers(skN1 *rlwe.SecretKey, galElOthers []uint64) (btpkeys *EvaluationKeys, skN2 *rlwe.SecretKey, err error) {
	...
	rlk := kgen.GenRelinearizationKeyNew(skN2)
	gks := kgen.GenGaloisKeysNew(append(p.GaloisElementsWithOther(paramsN2, galElOthers), paramsN2.GaloisElementForComplexConjugation()), skN2)
...
// GenEncapsulationEvaluationKeysNew generates the low level encapsulation EvaluationKeys for the aes_bootstrapping.
...
// writeEvkKey is a helper function for marshalling a aes_bootstrapping evaluation key.
...
// readEvkKey is a helper function for unmarshalling a aes_bootstrapping evaluation key.
