...
// of the aes_bootstrapping circuit.
type Parameters struct {
	// ResidualParameters: Parameters outside of the aes_bootstrapping circuit
	ResidualParameters ckks.Parameters
	// BootstrappingParameters: Parameters during the aes_bootstrapping circuit
	...
	// IterationsParameters: Parameters of the aes_bootstrapping iterations (META-BTS)
	IterationsParameters *IterationsParameters
	// EphemeralSecretWeight: Hamming weight of the ephemeral secret. If 0, no ephemeral secret is used during the aes_bootstrapping.
...
//
// The residualParameters corresponds to the [ckks.Parameters] that are left after the aes_bootstrapping circuit is evaluated.
// These are entirely independent of the aes_bootstrapping parameters with one exception: the ciphertext primes Qi must be
// congruent to 1 mod 2N of the aes_bootstrapping parameters (note that the auxiliary primes Pi do not need to be).
// This is required because the primes Qi of the residual parameters and the aes_bootstrapping parameters are the same between
...
// instantiating them.
//
// The method NewParametersFromLiteral will automatically allocate the [ckks.Parameters] of the aes_bootstrapping circuit based on
...
func NewParametersFromLiteral(residualParameters ckks.Parameters, btpLit ParametersLiteral) (Parameters, error) {
	...
	// Retrieve the LogN of the aes_bootstrapping circuit
	LogN := btpLit.GetLogN()
	...
	case ring.ConjugateInvariant:
		...
		// If ConjugateInvariant, then the aes_bootstrapping LogN must be at least 1 greater
		// than the residualParameters LogN
		if LogN != residualParameters.LogN()+1 {
			return Parameters{}, fmt.Errorf("cannot NewParametersFromLiteral: LogN of aes_bootstrapping parameters must be equal to LogN+ of residual parameters if ringtype is ConjugateInvariant")
		}

		// Takes the greatest NthRoot between the residualParameters NthRoot and the aes_bootstrapping NthRoot
		NthRoot = utils.Max(uint64(residualParameters.N()<<2), uint64(2<<LogN))

	default:

		// The LogN of the aes_bootstrapping parameters cannot be smaller than the LogN of the residualParameters.
		if LogN < residualParameters.LogN() {
			return Parameters{}, fmt.Errorf("cannot NewParametersFromLiteral: LogN of aes_bootstrapping parameters must be greater or equal to LogN of residual parameters")
		}
	...
	// Checks that all primes Qi of the residualParameters are congruent to 1 mod NthRoot of the aes_bootstrapping parameters.
	...
	// Number of aes_bootstrapping iterations
	var iterParams *IterationsParameters
	if iterParams, err = btpLit.GetIterationsParameters(); err != nil {
		return Parameters{}, err
	}

	// Boolean if there is a reserved prime for the aes_bootstrapping iterations
	...
	// List of the prime-size of all primes required by the aes_bootstrapping circuit.
	...
	// Retrieve the number of primes #Pi of the aes_bootstrapping circuit
	...
	// Instantiates the ckks.Parameters of the aes_bootstrapping circuit.
...
// DepthCoeffsToSlots returns the depth of the Coeffs to Slots of the aes_bootstrapping.
func (p Parameters) DepthCoeffsToSlots() (depth int) {
	return p.SlotsToCoeffsParameters.Depth(true)
}

// DepthEvalMod returns the depth of the EvalMod step of the aes_bootstrapping.
func (p Parameters) DepthEvalMod() (depth int) {
	return p.Mod1ParametersLiteral.Depth()
}

// DepthSlotsToCoeffs returns the depth of the Slots to Coeffs step of the aes_bootstrapping.
func (p Parameters) DepthSlotsToCoeffs() (depth int) {
	return p.CoeffsToSlotsParameters.Depth(true)
}

// Depth returns the depth of the full aes_bootstrapping circuit.
...
// GaloisElements returns the list of Galois elements required to evaluate the aes_bootstrapping.
...
// Chao add
// GaloisElements returns the list of Galois elements required to evaluate the aes_bootstrapping.
func (p Parameters) GaloisElementsWithOther(params ckks.Parameters, galElOthers []uint64) (galEls []uint64) {
